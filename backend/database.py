...
from .game_data import CARDS, GAME_DURATION, NUM_PAINS
...
HOSTED = bool(_configured_url)
if HOSTED:
    try:
        _url = make_url(_configured_url.replace("postgres://", "postgresql://", 1))
        if _url.get_backend_name() != "postgresql":
            raise ValueError()
        _url = _url.set(drivername="postgresql+psycopg")
        ENGINE = create_engine(
            _url, connect_args={"sslmode": "require", "connect_timeout": 10,
                                "options": "-c search_path=cloud_chaos", "prepare_threshold": None},
            pool_pre_ping=True, pool_size=3, max_overflow=2, pool_timeout=10,
...
def init_db() -> None:
    ...
    # Import models before create_all, including when invoked by standalone checks.
    from . import attempts, admin_auth  # noqa: F401
    if HOSTED:
        with ENGINE.begin() as conn:
            conn.execute(text("CREATE SCHEMA IF NOT EXISTS cloud_chaos"))
            conn.execute(text("REVOKE ALL ON SCHEMA cloud_chaos FROM PUBLIC"))
            conn.execute(text("REVOKE ALL ON SCHEMA cloud_chaos FROM anon, authenticated"))
...
def _sanitize_spreadsheet_values(df: pd.DataFrame) -> pd.DataFrame:
    """Neutralize formulas and export timezone-aware dates as UTC Excel dates."""
    sanitized = df.copy()
...
def save_game_session(
    ...
    for pain_idx, selection in selections.items():
        ...
        if selection["answer"] is not None and type(selection["answer"]) is not bool:
            raise ValueError("Answers must be boolean or unanswered.")
        ...
        try:
            with SessionFactory() as db:
                ...
                for pain_idx, sel in selections.items():
                    ...
                    is_correct = answer is not None and answer == CARDS[pain_idx]["is_right"]
                    db.add(Selection(
...
def get_leaderboard(limit: int = 20) -> list[dict[str, Any]]:
    ...
    """
    try:
        query = _get_leaderboard_cached.__wrapped__ if HOSTED else _get_leaderboard_cached
        return [dict(row) for row in query(limit)]
    except Exception:
        if HOSTED:
            raise
...
def get_admin_panel_data() -> list[dict[str, Any]]:
    ...
    """
    if HOSTED:
        return _get_admin_panel_data_from_db()
    try:
...
def get_stats() -> dict[str, Any]:
    ...
    """
    return dict(_get_stats_cached.__wrapped__() if HOSTED else _get_stats_cached())
    ...
    with SessionFactory() as db:
        totals = db.execute(text("""
            SELECT
                ...
                (SELECT COUNT(*) FROM game_sessions WHERE is_perfect=TRUE) AS perfect_count,
        ...
        pain_accuracy = db.execute(text("""
            SELECT
                pain_idx,
                COUNT(*)                                AS total,
                SUM(CASE WHEN both_correct=TRUE THEN 1 ELSE 0 END) AS correct_count,
                ROUND(
                    100.0 * SUM(CASE WHEN both_correct=TRUE THEN 1 ELSE 0 END) / COUNT(*),
                    1
                ) AS accuracy_pct
            FROM selections
            GROUP BY pain_idx
